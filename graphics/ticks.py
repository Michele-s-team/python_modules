    ...
    if scale == 'lin':
        ...
        if axis_direction_id == 0:
            ...
            if label != None:
                ax.text(value,
                        (origin[1] + length[1] * axis_origin[0]) -
                        tick_label_offset[0] * length[1],
                        label, fontsize=font_size, ha='center', va='center', zorder=z_order, rotation=tick_label_angle)
...
def plot_3d_tick(ax, axis_direction, value, label, tick_length, tick_label_offset, tick_label_format, origin, length,
    ...
    if axis_direction == 0:
        ...
        ax.plot(
            ...
            [
                gr.scale((origin[1] + length[1] * axis_origin[0]
                         [0]), origin[1], scale_factor[1]),
                gr.scale((origin[1] + length[1] * axis_origin[0][0]) + tick_length[0] *
                         length[1], (origin[1] + length[1] * axis_origin[0][0]), scale_factor[1])
            ],
            [gr.scale((origin[2] + length[2] * axis_origin[0][1]),
                      origin[2], scale_factor[2])] * 2,
        ...
        ax.plot(
            [
                gr.scale((origin[0] + length[0] * axis_origin[1]
                         [0]), origin[0], scale_factor[0]),
                gr.scale((origin[0] + length[0] * axis_origin[1][0]) + tick_length[1] *
                         length[0], (origin[0] + length[0] * axis_origin[1][0]), scale_factor[0])
            ],
            [gr.scale(value, origin[1], scale_factor[1])] * 2,
            [gr.scale((origin[2] + length[2] * axis_origin[1][1]),
                      origin[2], scale_factor[2])] * 2,
            color=color, linewidth=line_width, clip_on=False, zorder=z_order)
        ...
        ax.plot(
            ...
            [gr.scale((origin[1] + length[1] * axis_origin[2][1]),
                      origin[1], scale_factor[1])] * 2,
